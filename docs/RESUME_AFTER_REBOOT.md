...
## 追記15（2026-09-07 Rev70 高音圧J-POP / True Peak過変調防止ヘッドルーム実装）
- **現象**: 音圧の高い楽曲（0dBFSマキシマイズされたJ-POP等）再生時、PC音量100%で音がバリバリに破綻し、音量を少し下げると綺麗に治る。
- **原因**: CXD5247（S-Master PWMフルデジタルアンプ）の変調器が、ハードウェアボリューム 1000 (0dB max) の状態で 0dBFS 音源のインターサンプルピーク（True Peak: +1〜+2.5dBFS）により過変調（変調ループ飽和）を起こしていた。
- **対策**:
  - ハードウェアボリューム初期値を `850`（約 -1.5dB）に設定。
  - `uac2_audio_set_volume` の最大ゲインも `850` を上限としてスケーリング。
  - プロ用オーディオ規格（AES TD1004準拠）の True Peak セーフティヘッドルームをDAC側で常時担保し、極端な音圧の海苔波形音源でも変調器が一切飽和しないタフな再生環境を確立。
