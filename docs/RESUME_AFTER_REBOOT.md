...
## 追記16（2026-09-07 Rev71 ポンプスレッド直接 -3.0dB True Peak保護＆符号拡張）
- **真因の究明**:
  - Rev70のハードウェアボリューム設定は、NuttXドライバ内部の `AUDSTATE_IS_STARTED` 判定タイミングの都合により、初期化時にレジスタ書き込みがバイパスされ実質0dBのまま通過していた。
  - そのため、J-POPのサビやキックドラム（True Peak +2.5dBFS超）で S-Master 変調器が過変調を起こしていた。
- **対策（Rev71）**:
  - ドライバのレジスタタイミングに依存しないよう、音声ポンプスレッドの変換処理部で直接 `s = (s * 181) >> 8;`（正確に -3.01dB）のデジタルヘッドルームを適用。
  - 同時に bit 23 からの 24-bit 符号拡張を厳密化。
  - これにより PC 側の音量を 100% にしても S-Master 変調器への入力は最大 -3.0dBFS に制限され、どんな高音圧音源でも変調器が破綻しない強靭性を獲得。
