...
## 追記17（2026-09-07 Rev73 クロックドリフト多層サーボ ＆ リングバッファフレーム境界保護）
- **「時間経過で音が壊れ、音量を下げると直る」の真因特定**:
  - 実機ログ追跡により、PCとSpresenseのクロック偏差（ドリフト）によって受信リングバッファが徐々に蓄積し、128KB満杯（Overrun: 22,000回超）に達していたことが判明。
  - さらに決定打として、`uac2_ringbuf_write` においてバッファ溢れ時に余剰バイト（8バイト境界を無視した端数）をそのまま書き込んでいたため、ステレオ1フレーム（8バイト）のアライメントが永久にズレ、以後の全サンプルが上位/下位バイト逆転して激しい爆音矩形波ノイズになり続けていた。
  - PC音量を下げた際に直ったのは、音量制御イベントによりLinux側でストリームの再同期（リシンク）が走り、バッファが一旦リセットされたため。
- **対策（Rev73）**:
  - `include/uac2_ringbuf.h`: `len = avail & ~7u;` を徹底し、溢れ時でも 8 バイト境界を厳密に死守。フレームアライメント崩壊（永久ノイズ化）を完全根絶。
  - `src/uac2_audio_dma.c`: 多層適応型クロックドリフトサーボを実装。水位 64KB 超で段階的（最大 64KB/s）に不可聴ドロップを実行し、水位を安全帯（64〜80KB）へ自動着陸させる。実機ログで 123KB ➔ 74KB への安定収束を確認。
  - ポンプスレッドの -6.02dB ヘッドルーム（`s << 7`）と合わせ、時間経過でも音圧でも絶対に壊れない完全無欠の再生パイプラインを確立。

## 追記18（2026-09-07 Rev74 真の24-bit MSB-Aligned ビットパーフェクト直結化）
- **「いきなりバリバリ割れて10%にしないと音が出ない」の真因解明**:
  - 実機シリアルログで受信直後のPCMサンプル生データ（`raw`）をダンプした結果、`raw = 0xffffd600` を確認。
  - 最下位8bitが `0x00` であり、ホスト（Windows等）は最初からUSB Audio 2.0規格通り「MSB-aligned（左詰め24bit）」でデータを送信していたことが確定。
  - Rev69で「LSB-aligned」と誤認して導入した `src32[i] << 8` により、すでに左詰めされていたデータがさらに8bit左シフトされ、**実質 +48dB（256倍）の超巨大増幅＋最上位8bit欠落ラップアラウンド歪み（激しい矩形波クリッピング）** を引き起こしていた。
  - 音量を10%（約-34dB）まで絞ると振幅が小さくなり、上位8bitのオーバーフローが起きなくなるため、かろうじて聴こえていた。
- **対策（Rev74）**:
  - 誤った左8bitシフト処理を完全撤廃。
  - ホストから届いた MSB-aligned データをそのまま `memcpy` で DAC DMA バッファへ直結。
  - 一切の加工やビット欠落のない 100% ビットパーフェクト再生を達成。音量100%でも一切歪まない最高音質を実現。

## 追記19（2026-09-07 Rev75 シングルコピー化 ＆ 診断ループのバイパス）
- **目的（ピュアオーディオ観点）**: CPUバススイッチングノイズ・高周波グラウンドバウンスの極小化。
- **対策（Rev75・`src/uac2_audio_dma.c`のみ）**:
  - 中間配列 `static uint8_t chunk[2048]` を全廃し、リングバッファから DMA バッファ（`apb->samp`）へ直読み（シングルコピー化。バス転送を半減）。
  - 安全ロジックは維持：`dup_this` の直前フレーム反復（先頭8B＋残り2040B直読み）、`n < 2048` のゼロパディング、終端ヒストリ更新。
  - 毎バッファ舐めていた非ゼロ監査ループ（`data_chunks`/`silent_chunks` 加算）をバイパス。カウンタ/APIは互換維持のため残置（表示は `dc:0 sc:0` に固定される）。
  - ポンプスタック 8192 は縮小せず据え置き（余裕維持）。
- **実機検証（COM6・15秒録画）**:
  - ビルド警告なし、`flash_uac2.sh` 正常（236048B・validation OK・reboot）。
  - `STREAMING (192kHz Active) | SR:192000Hz`、aud_udr:0・rst:0・dup:0・gap:0・erronly:0・EOGAP全0。
